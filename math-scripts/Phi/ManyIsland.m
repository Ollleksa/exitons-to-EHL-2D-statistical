...
aSec[R_,S_,\[Phi]_,n_]:=-Vfi*(Gg[S]-c0*Exp[-\[Phi]/T+\[Alpha]/(R*T)])/(l*BesselK[1,R/l]+Vfi *(BesselK[0,R/l]+SumSec[S,n]));

aiSec[Ri_,R_,S_,\[Phi]_,n_]:=-Vfi*(Gg[S]+aSec[R,S,\[Phi],n]*SumSec[S,n]-c0*Exp[-\[Phi]/T+\[Alpha]/(Ri*T)])/(l*BesselK[1,Ri/l]+Vfi*BesselK[0,Ri/l]);

conSec[Ri_,R_,S_,\[Phi]_,n_]:=Gg[S]+aiSec[Ri,R,S,\[Phi],n]*BesselK[0,Ri/l]+aSec[R,S,\[Phi],n]*SumSec[S,n];

F1Sec[R_?NumericQ,S_?NumericQ,\[Phi]_,n_]:=n*NIntegrate[Log[(2*conSec[ x,R, S,\[Phi],n]*Vfi + x*GIsl[S])/(2*Vfi*c0*Exp[-\[Phi]/T+\[Alpha]/(x*T)] + x)]*x, {x, 0,R}];
...
ProbMany = Many[[1]];
If[Rmany>0,
PutAppend[ProbMany/10.^4, "Prob"<>ToString[n]<>"ManPhi"];
PutAppend[g,"g"<>ToString[n]<>"ManPhi"];
PutAppend[Rmany/100,"R"<>ToString[n]<>"ManPhi"];
PutAppend[Smany/100,"S"<>ToString[n]<>"ManPhi"]]
]
