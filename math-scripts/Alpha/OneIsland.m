...
aOne[R_,\[Alpha]_]:=-Vfi*(Gg[R]-c0*Exp[-\[Phi]/T+\[Alpha]/(R*T)])/(l*BesselK[1,R/l]+Vfi*BesselK[0,R/l]);
conOne[R_,\[Alpha]_]:=Gg[R]+aOne[R,\[Alpha]]*BesselK[0,R/l];

F1One[R_?NumericQ,\[Alpha]_]:=NIntegrate[Log[(2*conOne[x,\[Alpha]]*Vfi+ x*GIsl[x])/(2*Vfi*c0*Exp[-\[Phi]/T+\[Alpha]/(x*T)] + x)]*x, {x, 0,R}];
...
ProbOne = One[[1]];
If[ROne>0,
PutAppend[ProbOne/10.^4, "ProbOneAlpha"];
PutAppend[g,"gOneAlpha"];
PutAppend[ROne/100,"ROneAlpha"];]
]
